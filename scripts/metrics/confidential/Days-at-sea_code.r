...
### find unique trips and sum days at sea by year; SCHEDULE_NUMBER is unique ID for trips
tmp <- cflp[!duplicated(cflp$SCHEDULE_NUMBER),] 
days_year <- aggregate(tmp$DAYS_AWAY, by = list(tmp$LAND_YEAR), FUN = sum)
