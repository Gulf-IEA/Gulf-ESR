...
for (name in model_names) {
  ...
  # Check if timeseries data exists and has a 'Bio_all' column
  if (!is.null(base$timeseries) && "Bio_all" %in% colnames(base$timeseries)) {
    # Filter and process the biomass data, convert tons to million lbs
    biomass <- base$timeseries %>%
      filter(Era != "FORE") %>%
      filter(Yr >= 2000) %>%
      mutate(Bio_mil_lbs = (Bio_all * 2204.62) / 1000000)
    ...
    if (nrow(biomass) > 0) {
      ...
      if (length(unique(extent_names)) > 1) {
        name_map <- c("Area 1" = "East", "Area 2" = "West", "Area 3" = "Central")
        extent_names <- ifelse(extent_names %in% names(name_map), name_map[extent_names], extent_names)
      }
    ...
    warning(paste("Timeseries data or 'Bio_all' column not found for", name))
  }
