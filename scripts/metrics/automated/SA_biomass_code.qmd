  ...
  if (!is.null(base$timeseries) && "Bio_smry" %in% colnames(base$timeseries)) {
    ...
    if (nrow(biomass) > 0) {
      ...
      for (area in unique_areas) {
        ...
        file_name <- paste0(Species, "_Area", area, "_", Assessment, "_biomass.csv")
        
        write.csv(area_data, file = file.path(output_data_folder, file_name), row.names = FALSE)
      }
      
      # Save data for each assessment by area
      ...
      file_name = paste0(Species, "_", Assessment, "biomass_test.csv")
      write.csv(pivoted_biomass, file = file.path(output_data_folder, file_name), row.names = FALSE)
...
test just outputting data into IEA format
```{r}
# Define the output folder outside the loop
...
for (name in model_names) {
  ...
  if (!is.null(base$timeseries) && "Bio_smry" %in% colnames(base$timeseries)) {
    ...
    if (nrow(biomass) > 0) {
      ...
      indicator_text = paste0(Species, " ", Assessment, " ", "Biomass")
      ...
      file_name = paste0(Species, "_", Assessment, "biomass_formatted.csv")
