...
for (name in model_names) {
  ...
  if (length(commercial_land_fleets) > 0) {
    commercial_landings <- (rowSums(ts[, commercial_land_fleets, drop = FALSE], na.rm = TRUE))* 2.20462
  } else {
    commercial_landings <- rep(0, nrow(ts))
  }
  
  if (length(commercial_tot_fleets) > 0) {
    commercial_totcatch <- (rowSums(ts[, commercial_tot_fleets, drop = FALSE], na.rm = TRUE))* 2.20462
  } else {
  ...
  if (length(bycatch_disc_fleets) > 0) {
    bycatch_discards <- (rowSums(ts[, bycatch_disc_fleets, drop = FALSE], na.rm = TRUE))* 2.20462
  } else {
  ...
  unit_text_com = "Thousand pounds" #need to probably change this and check if correct
