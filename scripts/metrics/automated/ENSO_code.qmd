...
## Download Data
Data format description: https://psl.noaa.gov/data/timeseries/month/standard/

```{r}
#Roni data hosted online in text form
url<-"https://psl.noaa.gov/data/timeseries/month/data/roni.data"
col_nms<-c("year", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")

#Read in data using the online file
  ## no headers,
  ## skip the first line that says yearend,
  ## fill empties cause by descriptive rows at end of dataset,
  ## replace NA value with NAs
raw<-read.table(url, header = F,skip = 1, fill = T, na.strings = "-99.0", col.names = col_nms)

#Set all columns to numeric, make descriptive text NAs before removing
raw<-as.data.frame(lapply(raw, as.numeric))

#Remove descriptive and NA rows
...
## Format Data
```{r}
enso<-enso %>% pivot_longer(cols = -year)

enso$year<-paste(enso$name, enso$year, sep = "-")

enso$name<-NULL

#Remove leading and trailing NAs
enso<-enso[!is.na(enso$value),]
```
...
indicator_names = c("El Niño–Southern Oscillation")
unit_names = c("Relative Oceanic Niño Index")
extent_names = c("Gulf of America")

formatted_data = IEAnalyzeR::convert_cleaned_data(enso, indicator_names = indicator_names, unit_names = unit_names, extent_names = extent_names)
```
