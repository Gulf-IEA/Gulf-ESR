...
for (name in model_names) {
  ...
  if (!is.null(base$parameters)) {
    ...
    if (nrow(recdev3) > 0) {
      ...
      recdev3 = recdev3 %>% 
        select("Yr", "Value") #ignore the stdev for now
...
for (file in files) {
  ...
  file_name = paste0(Species, "_", Assessment, "_", "RecDev_object.rds")
...
for (file in files) {
  ...
  plot<-IEAnalyzeR::plot_fn_obj(df_obj = dat, manual_title = paste0(Species, " ", Assessment, " ", "RecDev"), trend = TRUE, xbreaks_by = 2, fig.width = 7)
  
  file_name = paste0(Species, "_", Assessment, "_", "RecDev_plot.png")
