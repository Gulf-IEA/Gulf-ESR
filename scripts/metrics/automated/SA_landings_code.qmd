...
for (file in files_com) {
  ...
  data_obj<-IEAnalyzeR::data_prep(dat)
...
for (file in files_rec) {
  ...
  process_status <- paste("ERROR processing", filename, "— skipped.")
  
  # Use tryCatch for the critical steps
  tryCatch({
    
    # 1. Attempt the data preparation
    data_obj <- IEAnalyzeR::data_prep(dat)
    
    # 2. If successful, save the RDS file
    file_name = paste0(Species, "_", Assessment, "_", "Recreational_Catch_object.rds")
    saveRDS(data_obj, file = file.path(output_data_folder, file_name))
    
    # 3. Update the status message upon success
    process_status <- paste("SUCCESS:", filename, "processed and saved.")
    
  }, error = function(e) {
    # This block executes ONLY if an error occurs and updates the error message
    ...
    # The 'process_status' variable retains the initial error message
  })
  
  # Print the final result (success or error) for each file
