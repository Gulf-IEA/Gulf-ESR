...
File created on 2026-01-08 by C. Gervasi
...
category_map <- list(
  "Inshore"    = c("Less than 4 hp", "4–9.9 hp", "10–29.9 hp", "30–49.9 hp"),
  "Nearshore" = c("50–74.9 hp", "75–99.9 hp", "100–149.9 hp"),
  "Near-offshore"   = c("150–199.9 hp", "200–249.9 hp", "250–299.9 hp"),
  "Far-offshore" = c("300 hp and greater")
)
...
outboard_summary <- hp_data_list %>%
  ...
  mutate(category = case_when(
    segment %in% category_map[["Inshore"]]    ~ "Inshore",
    segment %in% category_map[["Nearshore"]] ~ "Nearshore",
    segment %in% category_map[["Near-offshore"]]   ~ "Near-offshore",
    segment %in% category_map[["Far-offshore"]]   ~ "Far-offshore"
  )) %>%
  ...
  select(year, `Inshore`, `Nearshore`, `Near-offshore`, `Far-offshore`)
...
```{r}
#Other version of the indicator just far offshore by state
...
outboard_summary <- hp_data_list %>%
  ...
  filter(category == "Far-offshore") 

  
total_sales <- outboard_summary %>% 
  group_by(year, category) %>%
  summarise(FL = sum(Florida, na.rm = TRUE), AL = sum(Alabama, na.rm = TRUE), MS = sum(Mississippi, na.rm = TRUE), LA = sum(Louisiana, na.rm = TRUE), TX = sum(Texas, na.rm = TRUE), .groups = "drop") %>%
...
# indicator_names = rep("Outboard Engine Sales Trends", ncol(outboard_summary)-1)
# unit_names = c("<50 hp", "50-150 hp", "150-300 hp", "<300 hp")
# extent_names = c("Inshore", "Nearshore", "Near-offshore", "Far-offshore")

indicator_names = rep("Far Offshore Capable Outboard Engine Sales", ncol(total_sales)-1)
unit_names = rep(">300 hp units sold", ncol(total_sales)-1)
extent_names = c("Florida", "Alabama", "Mississippi", "Louisiana", "Texas")

formatted_data = IEAnalyzeR::convert_cleaned_data(total_sales, indicator_names, unit_names, extent_names)
```
...
```{r}
IEAnalyzeR::plot_fn_obj(df_obj = data_obj, trend = TRUE, xbreaks_by = 2, ncol = 1)
```
